...
在语义分析期间，实现应始终使用可用的最大精度来评估浮点指针值; 
...
只有非模糊符号才会添加到此范围。
但是总是可以通过写为 ``MyEnum.value`` 的类型限定来访问这些:
...
.. code-block:: nim
  ...
  type
    Person = tuple[name: string, age: int] # 代表一个人的类型：一个人由一个名字和一个年龄组成
  var
    person: Person
